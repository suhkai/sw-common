...
function getGlobalCall(errLine: string): LineInfo | never | undefined {
    ...
    try {
        ...
        const pathname = (matched[1] as string).startsWith('file') ? new URL(matched[1] as string).pathname : matched[1];
        ...
        throw new Error(`internal error #001: (see ReadMe) please file an issue: [${(err as unknown as Error).message}]`);
    }
}
...
function getInFunctionCall(errLine: string): LineInfo | never | undefined {
    ...
    try {
        ...
        // match browser or node
        const pathname = (matched[2] as string).startsWith('file') ? new URL(matched[2] as string).pathname : matched[2];
        if (!nonEmptyString(pathname)) {
        ...
        throw new Error(`internal error #002: (see ReadMe) please file an issue: [${(err as unknown as Error).message}]`);
    }
}
...
function getLineInfo(): LineInfo | never {
    ...
    if (lines[2] === undefined) {
        throw new Error(`internal error #003: (see ReadMe) please file an issue: [${(err as unknown as Error).message}]`);
    }
