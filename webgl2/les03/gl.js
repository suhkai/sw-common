...
function GLInstance(canvasID){
	...
	gl.fCreateArrayBuffer = function(floatAry,isStatic){
		...
		this.bindBuffer(this.ARRAY_BUFFER,buf); 										//put the "cursor" on this buffer
 		this.bufferData(this.ARRAY_BUFFER, floatAry, (isStatic)? this.STATIC_DRAW : this.DYNAMIC_DRAW ); // buffer operations
		this.bindBuffer(this.ARRAY_BUFFER,null); // remove cursor from the buffer
		return buf;
	}
	...
	gl.fCreateMeshVAO = function(name,aryInd,aryVert,aryNorm,aryUV){
		...
		//Create and bind vao
		rtn.vao = this.createVertexArray(); // vao															
